...
class RestaController extends CustomBaseController
{
    ...
    {
        $foodbills = foodbill::select('voucher_type','total_bill_value', 'total_qty', 'voucher_no','food_bill_no','service_id','voucher_date','status','user_id','customer_name','mobile', DB::raw('GROUP_CONCAT(voucher_no ORDER BY voucher_date SEPARATOR ",") as room_nos'))
        ->groupBy( 'voucher_type','voucher_no', 'total_bill_value', 'total_qty','food_bill_no','service_id','status','user_id','voucher_date','customer_name','mobile')
        ->orderBy('voucher_no','desc')
        ->where('voucher_type','Restaurant_food_bill')
        ->get(); 
        // $foodbills = foodbill::all();
    ...
    public function table_foodbill_create($tableid)
    {
        ...
        if ($kot_record > 0) {
           ...
           $voucher_no = $lastRecord->voucher_no;
           $new_voucher_no=$voucher_no+1;
           $voucher_type = voucher_type::where('voucher_type_name', 'Restaurant_food_bill')->first();
           $voucher_prefix=$voucher_type->voucher_prefix;
           $voucher_suffix=$voucher_type->voucher_suffix;
           $new_bill_no=$voucher_prefix."".$new_voucher_no."".$voucher_suffix;
    ...
    {    
        ...
        if($posting_acc_id>0){
            ...
            // return redirect()->url('table_foodbills')->with('message', 'Record saved successfully and bill amount posted to direct sale.');
            // $this->table_foodbill_print_view($request->voucher_no);
...
//    return view('entery.roomservice.foodbill.foodbill_print_view');
    ...
    public function destroy($voucher_no)
    {
        ...
        if ($foodbills->count()) {
            ...
            $ledger = ledger::where('transaction_type', 'Restaurant_food_bill')->where('voucher_no', $voucher_no);
            ...
            // Delete the foodbills
            $foodbills->delete();
    
            // Update the status of the corresponding KOT entry
            kot::where('voucher_no', $voucher_no)->update(['status' => '0']);
