...
class RoomcheckoutController extends CustomBaseController
{
    ...
    {
        // dd($request);
        $roomcheckout = roomcheckout::where('firm_id', Auth::user()->firm_id)
            ->where('voucher_no', $request->voucher_no)
            ->first();

        if ($roomcheckout) {
            $roomcheckins = roomcheckin::where('checkout_voucher_no', $request->voucher_no)
                ->where('firm_id', Auth::user()->firm_id)
                ->first();
        }    
            ...
            if ($roomcheckins) {
                ...
                ledger::where('account_id', $account_id)
                    ->where('firm_id', Auth::user()->firm_id)
            ...
            $checkinUpdated = RoomCheckin::where('firm_id', Auth::user()->firm_id)
    ...
    public function room_checkout_view2($voucher_no)
    {
        ...
        $roomcheckouts = roomcheckout::where('voucher_no', $voucher_no)
    ...
    public function edit($voucher_no)
    {
        ...
        $chekcout_data = roomcheckout::where('voucher_no', $id)
            ->where('firm_id', Auth::user()->firm_id)
            ->first();
        ...
        if ($roomcheckins->isNotEmpty()) {
            ...
            $checkout_record = roomcheckout::where('bill_type', 'Check_out')
                ->where('firm_id', Auth::user()->firm_id)
                ->count();
            if ($checkout_record > 0) {
                $lastRecord = roomcheckout::where('firm_id', Auth::user()->firm_id)
            ...
            $kots = kot::where('firm_id', Auth::user()->firm_id)
    ...
    {
        // dd($request);
        // $existingRecords = roomcheckout::where('firm_id', Auth::user()->firm_id)
        //     ->where('voucher_no', $request->voucher_no)
        //     ->count();
    ...
    {
        $roomcheckins = roomcheckin::where('checkout_voucher_no', $id)
            ->where('firm_id', Auth::user()->firm_id)->first();
    ...
    public function show_checkin(Request $request, $id)
    {
        ...
        if ($roomcheckins->isNotEmpty()) {
            ...
            // Pass all data except room_no and room_nos separately
            return view('entery.room.checkout.room_checkout_after_checkin', ['calculation_type' => $calculation_type, 'roomcheckins' => $roomcheckins, 'account_detail' => $account_detail, 'gst_percent' => $gst_percent, 'data' => $data, 'roomNos' => $roomNos, 'new_bill_no' => $new_bill_no, 'new_voucher_no' => $new_voucher_no, 'kots' => $kots, 'foodbills' => $foodbills, 'paymentmodes' => $paymentmodes, 'final_opning_balance' => $ledgerData['final_opning_balance'], 'bill_type' => $bill_type]);
            // dd( ['data' => $data, 'roomNos' => $roomNos]);
        } else {
            // Handle the case when no records are found
    ...
    //                 if ($checkin_date > $base_checkin_11am->modify('-1 hours')) {
    //                     $no_of_days++;
    //                 }
    ...
    public function show(Request $request, $calculation_type)
{
    ...
    public function destroy(string $id)
    {
        ...
        $roomsdetails = RoomCheckin::where('firm_id', Auth::user()->firm_id)
        ...
        if ($roomcheckout) {
            ...
            $ledger_sale = ledger::where('firm_id', Auth::user()->firm_id)
                ->where('transaction_type', 'Room_Checkout')
                ->where('voucher_no', $id);
            if ($ledger_sale) {
                // Update the room status to "vacant"
    ...
    public function My_Check_out()
    {
        ...
        $checkout_record = roomcheckout::where('bill_type', 'My_Check_out')->where('firm_id', Auth::user()->firm_id)->count();
        if ($checkout_record > 0) {
            $lastRecord = roomcheckout::where('bill_type', 'My_Check_out')->where('firm_id', Auth::user()->firm_id)->orderByRaw('CAST(voucher_no AS UNSIGNED) DESC')->first();
            $voucher_no = $lastRecord->voucher_no;
            $new_voucher_no = $voucher_no + 1;
