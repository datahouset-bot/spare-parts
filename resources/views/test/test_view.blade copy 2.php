...
<div class="container ">
    ...
    <div class="col-md-6 mt-2">
      ...
      <div class="form-floating mb-3 mb-md-0">
        ...
        <input type="text" name ="cust_name2" id="cust_name2" class="Form-control" >
        <div id ="cust_list">
...
$(document).ready(function(){
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.accountdata.length > 0) {
                    $.each(data.accountdata, function(index, account) {
                        $('#cust_name').append('<option value="' + account.id + '">' + account.account_name + '</option>');
                    });
                } else {
                    // No data returned
                    $('#cust_name').append('<option value="" disabled selected>No customers available</option>');
                }
            },
            error: function(err) {
    ...
    // Fetch customer data when the modal is opened
    // $('#myModal').on('shown.bs.modal', function () {
    //     fetchCustomerData();
    // });

    // Fetch customer data when the modal is closed
    ...
    //     // When select box is clicked, fetch customer data
    //     fetchCustomerData();
    // });
});
     ...
     <script>
         $(document).ready(function(){
          $("#cust_name2").on('keyup',function(){
            var value =$(this).val();
            $.ajax({

            type: "GET",
            url: "{{ url('\testshowform') }}",
            data:{'cust_name2':value},
            success:function(data){
              $("#cust_list").html(data.html);
