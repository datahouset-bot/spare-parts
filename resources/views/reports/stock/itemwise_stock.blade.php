          ...
          <form action="{{ url('item_wise_stock') }}" method="POST">
              ...
              <div class="row">
                  <div class="col-md-3 mb-3 mx-2">
                    Salect Account  
                    <select name="item_id" class="form-control" id="account_id" required>
                          <option value="" selected disabled>Select Account Name</option>
                          @foreach ($items as $record)
                          <option value="{{ $record->id }}">{{ $record->item_name }} &nbsp;::{{ $record->item_barcode }} </option>
                          @endforeach
                      </select>
                  </div>
                    ...
                    <button type="submit" class="btn btn-primary btn-block">OK</button>
                        ...
                        <table class ="table table-striped ">
                            <thead>
                             ...
                             <tbody>
                              @foreach ( $inveterydata as $record )
                              <tr>
