...
@section('pagecontent')
            ...
            <form id="item_entry">
                ...
                <div class="row">
                    ...
                    <div class="row my-2" name="kot_header">
                        ...
                        <div id="room_details_box" style="display: none;" class="col-md-2 col-4 text-center">
                            <select id="room_account_id" name="room_account_id">
                                <option value="Null" disabled selected>Select Room</option>
                                @foreach ($checkinlists as $checkinlist)
                                    <option value="{{ $checkinlist->voucher_no }}">
                                        {{ $checkinlist->guest_name }} || {{ $checkinlist->room_nos }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
    ...
    <script>
        $(document).ready(function() {
            $('#settle_payment').on('change', function() {
                if ($(this).val() === 'Room') {
                    $('#room_details_box').show();
                } else {
                    $('#room_details_box').hide();
                }
            });
        });
    </script>
    ...
    <script>
        $(document).ready(function() {
            ...
            function toggleFields() {
                ...
                // $('#Room_details_box').toggle(settlePayment === 'Room');
            }
                    ...
                    success: function(response) {
                        ...
                        if (response.status === 200) {
                            ...
                            itemRecords.forEach(function(record, index) {
                                ...
                                //                     if (record.item.item_group.toUpperCase() !== 'BAVRAGE') {
                                ...
                                $('#sold_item_record tbody').append(`
                                    <tr>
                                        ...
                                        <td>${parseFloat(itemdiscountamt).toFixed(2)}</td>
