  ...
  --}}
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#checkout_date, #checkin_date,#package_id').on('change', function() {
                ...
                // Start AJAX request
                $.ajax({
                    url: '/room-status', // Adjust URL according to your Laravel routes
                    type: 'POST',
                    data: data,
                    dataType: 'json',
                    success: function(response) {
                            ...
                            if (response.status === 200) {
                                ...
                                $.each(response.rooms, function(index, room) {
                                    ...
                                    var row = '<tr>' +
                                        '<td>' +
                                        '<label class="container_chekbox">' +
                                        '<input type="checkbox" class="room-checkbox" name="booking_room_id[]" value="' +
                                        room.id + '">' +
                                        '<span class="checkmark"></span>' +
                                        '</label>' +
                                        '</td>' +
...
$('#package_id').on('change', function() {
    if ($(this).val() !== '') {
        ...
        $('.room-checkbox').prop('disabled', true).prop('checked', false); // Also uncheck checkboxes
    }
});
