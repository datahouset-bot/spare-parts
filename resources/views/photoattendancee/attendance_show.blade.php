...
<img src="{{ asset('storage\app\public\room_image\\' . $employee->photo) }}" 
            ...
            <div class="col-md-8">
             ...
             <tr>
    <th>
        Salary ({{ date('F', mktime(0,0,0,$selectedMonth,1)) }})
    </th>
    <td>
        ₹{{ $employee->salary_amount }}

        <form method="POST"
              action="{{ route('salary.monthly.update', $employee->id) }}"
              class="d-inline ms-3">
            @csrf
            @method('PUT')
            ...
            <label>
                <input type="checkbox"
                       ...
                       {{ ($salaryPayment && $salaryPayment->status === 'paid') ? 'checked' : '' }}>
                <span class="fw-bold text-success">Paid</span>
            </label>
        </form>

        @if(!$salaryPayment || $salaryPayment->status === 'unpaid')
